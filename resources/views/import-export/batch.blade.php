...
@endif
<div class="card"><div class="card-header d-flex flex-wrap gap-2 align-items-center"><span class="me-2">Preview filter:</span>@foreach(['all' => 'All rows', 'warning' => 'Warnings', 'error' => 'Errors', 'duplicate' => 'Duplicates'] as $filter => $label)<button type="button" class="btn btn-sm {{ $filter === 'all' ? 'btn-primary' : 'btn-outline-primary' }} import-row-filter" data-filter="{{ $filter }}">{{ $label }}</button>@endforeach</div><div class="table-responsive"><table class="table table-sm mb-0"><thead><tr><th>Row</th>@if($batch->documentField())<th>{{ $batch->data_type === 'sales_invoices' ? 'Invoice' : 'Bill' }}</th><th>Date</th><th>{{ $batch->data_type === 'sales_invoices' ? 'Customer' : 'Supplier' }}</th><th>Branch</th><th>Item</th><th>Qty</th><th>Unit Price</th><th>Discount</th><th>Tax Code</th><th>Source Tax</th>@else<th>Data</th>@endif<th>Status</th><th>Warning / Error</th><th>Result</th><th>Details</th></tr></thead><tbody>
@php($previousDocument = null)
@foreach($batch->rows->take(config('imports.preview_rows')) as $row)
@php($values = $row->mapped_values ?: $row->raw_values)
@php($document = $batch->documentField() ? ($values[$batch->documentField()] ?? null) : null)
<tr class="import-preview-row {{ $document && $document !== $previousDocument ? 'table-group-divider' : '' }}" data-warning="{{ $row->warnings ? '1' : '0' }}" data-error="{{ $row->errors ? '1' : '0' }}" data-duplicate="{{ $row->duplicate_status !== 'not_duplicate' ? '1' : '0' }}"><td>{{ $row->source_row_number }}</td>
@if($batch->documentField())
<td class="fw-semibold">{{ $document === $previousDocument ? '↳' : $document }}</td><td>{{ $values[$batch->data_type === 'sales_invoices' ? 'invoice_date' : 'bill_date'] ?? '—' }}</td><td>{{ $values[$batch->data_type === 'sales_invoices' ? 'customer' : 'supplier'] ?? '—' }}</td><td>{{ $values['branch'] ?? '—' }}</td><td>{{ $values['item'] ?? '—' }}</td><td>{{ $values['quantity'] ?? '—' }}</td><td>{{ $values['unit_price'] ?? '—' }}</td><td>{{ $values['discount'] ?? '—' }}</td><td>{{ $values['tax_code'] ?? '—' }}</td><td>{{ $values['source_tax'] ?? '—' }}</td>
@else
<td>@foreach($values as $key => $value)<span class="d-inline-block me-2"><span class="text-muted">{{ str($key)->replace('_', ' ')->title() }}:</span> {{ is_scalar($value) ? $value : json_encode($value) }}</span>@endforeach</td>
@endif
<td>{{ str($row->validation_status)->replace('_', ' ')->title() }}@if($row->duplicate_status !== 'not_duplicate')<div class="text-muted small">Duplicate</div>@endif</td><td class="{{ $row->errors ? 'text-danger' : 'text-warning-emphasis' }}">{{ implode('; ', [...$row->warnings, ...$row->errors]) }}</td><td>{{ $row->result_model ? class_basename($row->result_model).' #'.$row->result_id : '—' }}</td><td><details><summary>Mapped data</summary><pre class="small mb-0">{{ json_encode($values, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre></details></td></tr>
@php($previousDocument = $document)
@endforeach
</tbody></table></div></div>
@endif
...
document.getElementById('compatible-profile')?.addEventListener('change', function () {
    ...
    const mapping = JSON.parse(atob(this.value));
    document.querySelectorAll('.import-mapping').forEach(select => { select.value = mapping[select.dataset.source] || ''; });
});
document.querySelectorAll('.import-row-filter').forEach(button => button.addEventListener('click', function () {
    const filter = this.dataset.filter;
    document.querySelectorAll('.import-row-filter').forEach(item => {
    ...
    document.querySelectorAll('.import-preview-row').forEach(row => {
        row.hidden = filter !== 'all' && row.dataset[filter] !== '1';
    });
}));
</script>
@endsection
